# @lerobot-viewer/player — 多模态数据播放器 SDK 设计文档

**中文 | [English](DESIGN.md)**

> 注意：这份文档描述的是 SDK 的初始形态。随着代码演进，某些细节（如 STL 缓存作用域、`RobotViewerPanel` 的 `theme` prop、视频漂移纠偏节流频率）已在英文版 `DESIGN.md` 中同步更新。中文版保留作参考。

## 1. 概述

`@lerobot-viewer/player` 是一个面向 [LeRobot](https://github.com/huggingface/lerobot) 数据集的多模态回放 SDK，用于在 Web 端同步播放机器人采集的多路数据。
